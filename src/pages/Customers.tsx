...
import { Plus, Search, MapPin, Phone, ChevronRight, Wrench, X, Pencil, Trash2 } from 'lucide-react';

const EMPTY_CUSTOMER = { fullName: '', phone: '', region: '', zone: '', woreda: '', specificLocation: '', latitude: '', longitude: '', notes: '' };
const EMPTY_INSTALL = {
  projectTitle: '', status: 'Pending', deliveredBy: '', receivedBy: '', installationDate: '', remarks: '',
  wellData: { diameter: '', depth: '', waterLevel: '', casingSize: '', casingType: '' },
  activitiesPerformed: { casing: false, solarPump: false, testing: false, solarPanelStructure: false, sprinkler: false, practicalTraining: false },
};
const ACTIVITIES = [
...
export default function Customers() {
  ...
  const openEdit = (c: any) => {
    setForm({ fullName: c.fullName, phone: c.phone || '', region: c.region, zone: c.zone || '', woreda: c.woreda || '', specificLocation: c.specificLocation || '', latitude: c.latitude ?? '', longitude: c.longitude ?? '', notes: c.notes || '' });
    setEditCustomer(c);
    setError('');
  };
  ...
  const saveCustomer = async () => {
    ...
    try {
      const body = { ...form, latitude: form.latitude !== '' ? Number(form.latitude) : null, longitude: form.longitude !== '' ? Number(form.longitude) : null };
      if (editCustomer) {
        await cApi.update(editCustomer._id, body);
      } else {
        await cApi.create(body);
      }
      setShowCreate(false); setEditCustomer(null); setForm({ ...EMPTY_CUSTOMER }); load();
    } catch (e: any) {
  ...
  const selectCustomer = async (c: any) => {
    setSelectedCustomer(c); setLoadingInstalls(true); setCustomerInstalls([]);
  ...
  const saveInstallation = async () => {
    ...
    try {
      ...
      const r = await cApi.installations(selectedCustomer._id);
      setCustomerInstalls(r.data.installations);
    } catch (e: any) {
    ...
    <Layout>
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          ...
          <button className="btn-primary flex items-center gap-2" onClick={() => { setForm({ ...EMPTY_CUSTOMER }); setError(''); setShowCreate(true); }}>
          ...
          <div className="flex-1 space-y-2">
            {loading ? (
              ...
              <div
                ...
                className={`card cursor-pointer transition-all hover:border-primary/50 ${selectedCustomer?._id === c._id ? 'border-primary bg-primary/5' : ''}`}
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={(e) => { e.stopPropagation(); openEdit(c); }} className="p-1.5 text-gray-500 hover:text-primary rounded-lg hover:bg-primary/10 transition-colors">
                      <Pencil className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); setDeleteTarget(c); }} className="p-1.5 text-gray-500 hover:text-red-400 rounded-lg hover:bg-red-900/20 transition-colors">
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                    <ChevronRight className={`w-4 h-4 transition-transform ${selectedCustomer?._id === c._id ? 'text-primary rotate-90' : 'text-gray-600'}`} />
                  </div>
                </div>
              </div>
            ))}
            ...
            <div className="w-96 shrink-0 space-y-3">
              <div className="card">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-white text-sm">{selectedCustomer.fullName}</h3>
                  <button onClick={() => setSelectedCustomer(null)} className="text-gray-500 hover:text-white"><X className="w-4 h-4" /></button>
                </div>
                {selectedCustomer.specificLocation && (
                  <p className="text-xs text-gray-400 mb-2">{selectedCustomer.specificLocation}</p>
                )}
                <button className="btn-primary w-full flex items-center justify-center gap-2 mt-2"
                  onClick={() => { setInstallForm({ ...EMPTY_INSTALL }); setInstallError(''); setShowInstallForm(true); }}>
                  <Wrench className="w-4 h-4" /> New Installation
                </button>
              </div>
              ...
              <div className="card">
                ...
                {loadingInstalls ? (
                  ...
                  <p className="text-gray-600 text-sm text-center py-4">No installations yet</p>
                ) : (
      ...
      {(showCreate || editCustomer) && (
        <Modal title={editCustomer ? 'Edit Customer' : 'Add Customer'} onClose={() => { setShowCreate(false); setEditCustomer(null); }}>
          {error && <p className="mb-3 text-sm text-red-400 bg-red-900/20 border border-red-700/40 px-3 py-2 rounded-lg">{error}</p>}
          <div className="space-y-4">
            ...
            <div className="flex gap-3 pt-1">
              <button className="btn-ghost flex-1" onClick={() => { setShowCreate(false); setEditCustomer(null); }}>Cancel</button>
      ...
      {/* Installation Modal */}
      {showInstallForm && (
        <Modal title="Installation & Handover Form" onClose={() => setShowInstallForm(false)} wide>
          {installError && <p className="mb-3 text-sm text-red-400 bg-red-900/20 border border-red-700/40 px-3 py-2 rounded-lg">{installError}</p>}
          <div className="space-y-4">
            <div>
              <label className="form-label">Customer</label>
              <input className="form-input bg-surface-border/30 cursor-not-allowed" value={selectedCustomer.fullName} readOnly />
            </div>
            <div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[['diameter','Diameter (m)'],['depth','Depth (m)'],['waterLevel','Water Level (m)'],['casingSize','Casing Size']].map(([k, label]) => (
                  <div key={k}>
                    <label className="form-label">{label}</label>
                    <input className="form-input" value={installForm.wellData[k]} onChange={(e) => setInstallForm({ ...installForm, wellData: { ...installForm.wellData, [k]: e.target.value } })} />
                  </div>
                ))}
                <div className="col-span-2">
                  <label className="form-label">Casing Type</label>
                  <input className="form-input" value={installForm.wellData.casingType} onChange={(e) => setInstallForm({ ...installForm, wellData: { ...installForm.wellData, casingType: e.target.value } })} />
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <textarea className="form-input" rows={3} value={installForm.remarks} onChange={(e) => setInstallForm({ ...installForm, remarks: e.target.value })} />
            </div>
            <div className="flex gap-3 pt-1">
              <button className="btn-ghost flex-1" onClick={() => setShowInstallForm(false)}>Cancel</button>
      ...
      {deleteTarget && (
        <Modal title="Delete Customer" onClose={() => setDeleteTarget(null)}>
          <p className="text-sm text-gray-300 mb-5">Delete <span className="text-white font-semibold">{deleteTarget.fullName}</span> and all their installations? This cannot be undone.</p>
